...
static func describe(unit: Unit) -> Dictionary:
	...
	if unit.combat_form_id == form.target_form:
		return {"subtitle": "Ennemi · Démon de feu", "heading": "Métamorphose accomplie",
			"description": "Paris a abandonné son arc pour son fouet de feu. La métamorphose lui a rendu tous ses PV et accordé 30 bouclier. Sa forme est définitive ; cet effet ne se répète pas.",
			"future_spells": ""}
	...
	return {"subtitle": "Ennemi · Archer spectral", "heading": "Métamorphose — sous %d %% de PV" % form.below_hp_percent,
		"description": "Lorsqu’il survit à un coup sous %d %% de ses PV initiaux, Paris devient un démon de feu, récupère tous ses PV et reçoit %d bouclier. Une seule fois, en conservant ses PA/PM restants. Un coup fatal le tue normalement." % [form.below_hp_percent, form.shield_grant],
