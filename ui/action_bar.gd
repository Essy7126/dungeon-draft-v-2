...
func _build_ui() -> void:
	...
	# PA en pips entiers (pleins/vides) : le tour se lit d'un coup d'oeil.
	_ap_label = Label.new()
	...
	resource_vbox.add_child(_ap_label)
	...
	_move_btn.tooltip_text = "PM : sert uniquement au deplacement. PA : paie les attaques et les sorts."
	...
	_attack_btn.tooltip_text = "Attaque de base : coute 1 PA pour frapper au contact."
...
func _can_use_spell(unit, spell, imprinted: bool = false) -> bool:
	if unit == null or spell == null:
		return false
	return unit.can_afford_spell_resources(spell, imprinted)
...
func _ap_pips(current: int, max_value: int) -> String:
	var shown_max: int = maxi(max_value, 0)
	var filled: int = clampi(current, 0, shown_max)
