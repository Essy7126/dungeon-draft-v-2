...
const MAX_SPELL_XP_PER_COMBAT := 5
const MAX_DISCIPLINE_XP_PER_COMBAT := MAX_SPELL_XP_PER_COMBAT
...
func grant_cast_xp(
	...
	if spell_id == &"spell:unassigned":
		return _refusal(&"invalid_spell_id", caster, spell)
	...
	if character_state.get_spell_progress(spell_id) == null:
		return _refusal(&"no_skill_tree", caster, spell, character_state.character_id)
	var combat_key := _combat_key(
		character_state.character_id,
		spell_id,
	)
	var combat_xp := int(_combat_xp_by_key.get(combat_key, 0))
	if combat_xp >= MAX_SPELL_XP_PER_COMBAT:
		return _refusal(
			&"combat_cap_reached",
			caster,
			spell,
	...
	var progress_result := character_state.add_spell_xp(spell_id, 1)
	if progress_result.is_empty():
		return _refusal(
			&"spell_progress_missing",
			caster,
			spell,
			character_state.character_id,
			combat_xp,
	...
	progress_result["discipline_id"] = spell.get_effective_spell_id()
	...
	progress_result["combat_cap"] = MAX_SPELL_XP_PER_COMBAT
...
func get_combat_xp(character_id: StringName, spell_id: StringName) -> int:
	return int(_combat_xp_by_key.get(_combat_key(character_id, spell_id), 0))


func _combat_key(character_id: StringName, spell_id: StringName) -> String:
	return "%s|%s" % [str(character_id), str(spell_id)]
...
func _refusal(
	...
	return {
		...
		"discipline_id": spell.get_effective_spell_id() if spell != null else &"",
		"caster": caster,
		"spell_id": spell.get_effective_spell_id() if spell != null else &"",
		"activation_index": caster.activation_index if caster != null else -1,
		"activation_xp": 0,
		"combat_xp": combat_xp,
		"combat_cap": MAX_SPELL_XP_PER_COMBAT,
		"combat_cap_reached": combat_xp >= MAX_SPELL_XP_PER_COMBAT,
	}
