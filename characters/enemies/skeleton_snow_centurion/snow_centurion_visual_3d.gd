...
const ANIM_ATTACK: StringName = &"DD_SnowCenturion_Attack"
const ANIM_HEAVY_ATTACK: StringName = &"DD_SnowCenturion_HeavyAttack"
...
const ATTACK_IMPACT_NORMALIZED := 0.244444
...
func _create_hand_effect_origin(
	...
	if skeleton == null or skeleton.find_bone(str(bone_name)) < 0:
		push_warning("SnowCenturionVisual3D: os %s absent, origine d'effet non creee." % bone_name)
		return
