class_name CatabaseMonsterIsoUnitView
extends Node2D
...
@export var sprite_profile: CatabaseMonsterSpriteProfile
...
func configure_profile(profile: CatabaseMonsterSpriteProfile) -> bool:
	...
	if _dead:
		_stem = "death"
		_sample_normalized("death", _death_elapsed / sprite_profile.death_duration_seconds)
	else:
		_hold_idle_pose()
	_reset_clock()
	return true
...
func update_movement_stride(step_index: int, progress: float) -> void:
	if not _can_act() or not _movement_active:
		return
	# Battle progress is the only gait clock for path movement. A cell boundary
	# preserves the cycle and every morphology advances only as its root travels.
	_movement_phase = fposmod((float(maxi(step_index, 0)) + clampf(progress, 0.0, 1.0)) \
		* sprite_profile.stride_cycles_per_cell, 1.0)
	_sample_walk()
...
func get_spell_animation_stem(spell: Spell) -> String:
	return sprite_profile.spell_animation_stem(spell) if sprite_profile != null else "cast"
...
func _can_act() -> bool:
	return _configured and not _closing and not is_queued_for_deletion() and not _dead and not _action_pending
...
func _begin_action(action_id: StringName, stem: String) -> bool:
	...
	_reset_clock()
	if sprite_profile.release_frame == 0:
		_advance_action(0.0)
	return true
...
func _release_seconds() -> float:
	if not _action_pending or sprite_profile == null or _frames == null:
		return 0.0
	var clip := StringName(_stem + "_" + _facing)
	var preceding_weight := 0.0
	for index in sprite_profile.release_frame:
		preceding_weight += _frames.get_frame_duration(clip, index)
	return _action_duration * preceding_weight / _clip_weight(clip)
...
func _track_parent_movement(delta: float) -> void:
	...
	if distance > 0.01:
		...
		if not _movement_feedback_owned:
			...
			_movement_phase = fposmod(_movement_phase \
				+ distance / sprite_profile.external_stride_distance_pixels, 1.0)
			_sample_walk()
		return
