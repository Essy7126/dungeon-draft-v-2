...
func test_relic_draft_publication_and_reward_eligibility_are_isolated() -> void:
	...
	publication.save_service.transaction_root = TEST_ROOT + "/publication/transactions"
	...
	draft_service.save_service.transaction_root = TEST_ROOT + "/publication/transactions"
	var draft := draft_service.save_draft(document, catalog)
	assert_true(draft.get("ok", false), str(draft))
	assert_true(draft.get("not_in_production_catalog", false))
	var published := publication.publish(document, catalog, true)
