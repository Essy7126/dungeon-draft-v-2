...
func test_formule_finale_est_par_famille_fois_salle_avec_caps_explicites() -> void:
	...
	assert_almost_eq(forest.final_visual_scale(&"skeleton_melee"), 1.659, 0.0001)
	assert_almost_eq(forest.final_visual_scale(&"odyssey_skirmisher"), 1.659, 0.0001)
	assert_almost_eq(forest.final_visual_scale(&"odyssey_guard"), 1.659, 0.0001)
	assert_almost_eq(volcano.final_visual_scale(&"elf"), 1.8576, 0.0001)
	assert_almost_eq(volcano.final_visual_scale(&"skeleton_chief"), 1.7064, 0.0001)
	assert_almost_eq(volcano.final_visual_scale(&"odyssey_champion"), 1.7064, 0.0001)
	...
	assert_almost_eq(space.final_visual_scale(&"skeleton_melee"), 1.738, 0.0001)
	assert_almost_eq(space.final_visual_scale(&"skeleton_chief"), 1.738, 0.0001)
...
func test_echelles_familles_appliquent_le_second_palier_de_lisibilite() -> void:
	...
	for unit_id in [&"skeleton_melee", &"skeleton_ranged"]:
		var family := profile.profile_for_unit(unit_id)
		assert_gte(family.final_visual_scale(profile.global_unit_scale_multiplier), 1.73)
		assert_lte(family.final_visual_scale(profile.global_unit_scale_multiplier), 1.75)
	var elite := profile.profile_for_unit(&"skeleton_chief")
	assert_gte(elite.final_visual_scale(profile.global_unit_scale_multiplier), 1.73)
	assert_lte(elite.final_visual_scale(profile.global_unit_scale_multiplier), 1.75)
