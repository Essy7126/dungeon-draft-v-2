...
func test_three_states_units_loadouts_xp_and_modifiers_persist_between_rooms() -> void:
	...
	units[1].current_energy = 17.0
	units[2].current_energy = 23.0
	...
	assert_eq(units[1].current_energy, 17.0)
	assert_eq(units[2].current_energy, 23.0)
...
func test_solo_to_three_rebuilds_the_full_order_without_old_keys() -> void:
	...
	var states := _prepare_party()
	assert_eq(states.map(func(state): return state.unit.unit_name), ["Elfe", "Gardien", "Guerrier"])
