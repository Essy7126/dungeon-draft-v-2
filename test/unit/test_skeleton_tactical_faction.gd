...
func test_telegraph_layer_tracks_prepare_and_resolution_lifecycle() -> void:
	...
	assert_true(target.moved.is_connected(layer._on_tracked_unit_moved))
	assert_true(field.grid.relocate_unit(target, Vector2i(3, 0)))
	chief.start_turn()
	field.caster.resolve_pending_activation(chief)
	assert_eq(layer.get_telegraph_count(), 0)
	assert_false(target.moved.is_connected(layer._on_tracked_unit_moved))
