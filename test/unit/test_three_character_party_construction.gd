...
func test_turn_queue_receives_fixed_party_in_logical_order() -> void:
	...
	queue.setup(heroes)
	queue.start()
	assert_eq(queue.get_upcoming_order(), [heroes[0], heroes[1], heroes[2]])
	queue.advance()
	assert_eq(queue.get_upcoming_order(), [heroes[1], heroes[2], heroes[0]])
	queue.advance()
	assert_eq(turns, ["Elfe", "Mage", "Guerrier"])
	assert_same(queue.get_current_unit(), heroes[2])
	assert_eq(queue.get_upcoming_order(), [heroes[2], heroes[0], heroes[1]])
