...
func _ready() -> void:
	for region: Dictionary in definition.get("mist", []):
		...
		_add_mist(Vector2(r[0], r[1]) * extent, Vector2(r[2], r[3]) * extent,
			Color(str(region.color), float(region.alpha)), false)
...
func _draw() -> void:
	...
	if fire_on:
		for i: int in definition.torches.size():
			...
			for j: int in 7:
				...
				var color := Color(1.0, 0.55 + j * 0.035, 0.14, sin(age * PI) * (1.0 - age) * 0.9 * amount)
	...
	if water_on:
		for i: int in definition.cascades.size():
			...
			for j: int in 10:
				...
				var at := source + Vector2(side * width * age, -38.0 * age * (1.0 - age) + age * 11.0)
				draw_circle(at, 1.1, Color(0.66, 1.0, 0.83, sin(age * PI) * 0.52 * amount), true, -1, true)
			var cycle := fposmod(clock * 0.48 + i * 0.231, 1.0)
			var ring := PackedVector2Array()
			for j: int in 33:
				var angle := j / 32.0 * TAU
				ring.append(source + Vector2(cos(angle), sin(angle) * 0.37) * (cycle * width * 1.65 + 3.0))
			draw_polyline(ring, Color(0.54, 1.0, 0.76, sin(cycle * PI) * (1.0 - cycle) * 0.23 * amount), 0.9, true)
	...
	for i: int in 28:
		...
		var at := Vector2(x, 0.28 + fposmod(i * 0.217, 0.48)) * extent + Vector2(sin(clock * 0.4 + phase) * 18, -age * 55)
		draw_circle(at, 1.1, Color(0.74, 0.96, 0.56, pow(sin(age * PI), 2) * 0.48 * amount), true, -1, true)
