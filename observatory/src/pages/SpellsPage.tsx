...
import { labelDamageType, labelEffectTag, labelElement } from '../data/translations';
  ...
  return (
    <>
      ...
      <form className="filter-panel" onSubmit={(event) => event.preventDefault()} aria-label="Filtres des sorts">
        ...
        <label>Effet<select value={filters.effect} onChange={(event) => update('effect', event.target.value)}><option value="">Tous</option>{['damage', 'heal', 'shield', 'terrain', 'status', 'push', 'pull', 'summon', 'delayed'].map((value) => <option key={value} value={value}>{labelEffectTag(value)}</option>)}</select></label>
        <label>Type de dégâts<select value={filters.damageType} onChange={(event) => update('damageType', event.target.value)}><option value="">Tous</option>{uniqueSorted(snapshot.spells.map((spell) => spell.damage_type.name)).map((value) => <option key={value} value={value}>{labelDamageType(value)}</option>)}</select></label>
        <label>Élément<select value={filters.element} onChange={(event) => update('element', event.target.value)}><option value="">Tous</option>{uniqueSorted(snapshot.spells.map((spell) => spell.element.name)).map((value) => <option key={value} value={value}>{labelElement(value)}</option>)}</select></label>
          ...
          {spells.map((spell) => (
            <article className="entity-card compact-card" key={spell.id}>
              <div className="entity-card__body">
                ...
                <div className="effect-list">{spellEffects(spell).map((effect) => <EffectBadge key={effect}>{labelEffectTag(effect)}</EffectBadge>)}</div>
