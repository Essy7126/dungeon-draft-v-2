...
export function EnemiesPage() {
  ...
  const profilesById = new Map(snapshot.ai_profiles.map((profile) => [profile.id, profile]));
  const enemies = selectEnemies(snapshot.enemies, filters, profilesById);
  ...
  return (
    <>
      ...
      <section className="filter-panel enemy-filters" aria-label="Filtres des ennemis">
        ...
        <label>Rôle<select value={filters.role} onChange={(event) => set('role', event.target.value)}><option value="">Tous</option>{uniqueSorted(snapshot.enemies.map((enemy) => enemy.tactical_role_id)).map((value) => <option key={value}>{value}</option>)}</select></label>
        <label>Stratégie IA<select value={filters.aiStrategy} onChange={(event) => set('aiStrategy', event.target.value)}><option value="">Toutes</option>{uniqueSorted(snapshot.ai_profiles.map((profile) => profile.strategy.name)).map((value) => <option key={value}>{value}</option>)}</select></label>
      ...
      {enemies.length === 0 ? <EmptyState title="Aucun ennemi trouvé">Modifiez ou réinitialisez les filtres.</EmptyState> : <div className="entity-grid enemy-grid">{enemies.map((enemy) => {
        ...
        return <article className="entity-card enemy-card" key={enemy.id}>
          ...
          <div className="entity-card__body">
            ...
            <p className="relation-line">{enemy.tactical_role_id} · {profile?.strategy.description ?? enemy.ai_profile_id}</p>
            <div className="effect-list">{enemy.effect_tags.map((tag) => <EffectBadge key={tag}>{tag.replace('_', ' ')}</EffectBadge>)}</div>
