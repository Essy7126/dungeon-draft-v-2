...
async function main() {
  ...
  const manifestPath = path.resolve(process.argv[2] ||
    'artifacts/spectre_sprite_validation_v1/gameplay_clip/clip/clip_manifest.json');
  ...
  const outputPath = path.join(directory, 'spectre_gameplay.gif');
  ...
  const poster = frames.find(frame => frame.animation.startsWith('attack_') && frame.frame >= 3) || frames[0];
