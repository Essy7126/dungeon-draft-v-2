"""Build the grounded processional path in the guarded private Blender lab."""
import json
import math
...
OUT = ROOT / "art/source/halts/underworld_threshold_v1/grounded"
OUT.mkdir(parents=True, exist_ok=True)
(OUT / ".gdignore").write_text("", encoding="utf-8")
scene = bpy.context.scene
assert scene.get("halt_lab_id") == "dungeon_draft_halt_scale_lab_v1"
scene.name = "Le Seuil des Ombres — voie funéraire"
scene["halt_export_collection"] = "Threshold grounded generated"
scene["halt_export_directory"] = "art/source/halts/underworld_threshold_v1/grounded"
scene["halt_required_groups"] = ["statue_left", "statue_right", "statue_rear"]
for sibling in scene.collection.children:
    if sibling.name != "Threshold grounded generated":
        sibling.hide_render = True
        sibling.hide_viewport = True
old = bpy.data.collections.get("Threshold grounded generated")
if old:
    for obj in list(old.objects):
        bpy.data.objects.remove(obj, do_unlink=True)
    bpy.data.collections.remove(old)
collection = bpy.data.collections.new("Threshold grounded generated")
...
# Continuous ground extends outside the camera; no terrace, platform or parapet.
block("Continuous earth", (0,0,-.18), (34,30,.25), "dark", "floor", 0)
# A wide, curving processional lane joins a loose clearing at the gate.
centres=[(-7,-6),(-5.5,-4),(-3.4,-2.7),(-1.5,-1.7),(.2,-.6),(1.5,1),(2.5,2.4)]
for i in range(-10,9):
    for j in range(-9,6):
        x,y=i*.8,j*.8
        near=min((x-cx)**2+(y-cy)**2 for cx,cy in centres)
        clearing=((x+.3)/5.4)**2+((y-.3)/3.4)**2 < 1
        if near<3.1 or clearing:
            tile=block(f"Irregular buried flagstone {i}-{j}",(x,y,-.025),(.73,.74,.08),"floor","floor",.05)
            tile.rotation_euler.z=.06*math.sin(i*4+j*3)
# Crag wall encloses the back of the ancient gate instead of standing free.
for index,(x,y,z,sx,sy,sz) in enumerate([
    (-5.4,3.7,1.5,1.6,1.5,2.0),(-3.7,4.4,1.1,1.4,1.3,1.6),
    (-.2,6.5,2.0,1.4,1.4,2.6),(2.4,7.1,2.7,1.8,1.5,3.1),
    (5.6,6.8,2.7,1.5,1.4,3.1),(7.5,5.1,2.2,1.4,1.5,2.7),
    (8.5,3.0,1.3,1.5,1.8,1.8),(6.4,-2.3,.65,1.2,1.3,.9),
    (-6.2,-.7,.6,1.3,1.6,.9),(-3.4,-5.1,.5,1.6,1.0,.75)]):
    rock=sphere("Rock outcrop "+str(index),(x,y,z),(sx,sy,sz),"stone","rock")
    rock.rotation_euler=(.14*index,.1*index,.4*index)
# Rooted trees at side margins, with spreading roots along the continuous soil.
for x,y,h in [(-6.7,1.8,6.2),(9.0,1.3,5.8),(-4.8,5.3,5.9)]:
    cone("Ancient rooted trunk",(x,y,h/2),.5,.28,h,"bronze","roots",12)
    for dx,dy in [(1.4,-.7),(-1.3,-1.0),(.7,1.6)]:
        limb("Root", (x,y,.4),(x+dx,y+dy,.04),.20,"roots")
    limb("Branch",(x,y,h*.6),(x-1.5,y+.6,h*.9),.2,"roots")
