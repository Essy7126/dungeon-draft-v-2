...
async function main() {
  ...
  const manifestPath = path.resolve(process.argv[2] ||
    'artifacts/achilles_kit_sprite_validation_v2/clip/clip_manifest.json');
  ...
  const outputPath = path.join(directory, 'achilles_kit_gameplay.gif');
  ...
  const poster = frames.find(frame => frame.hero_animation.startsWith('bow_') || frame.hero_animation.startsWith('sweep_')) || frames[0];
