...
$root = Get-DevRoot
$reports = New-DevRun 'camp-companions-play'
...
foreach ($key in @('APPDATA', 'LOCALAPPDATA', 'XDG_DATA_HOME')) {
    $start.Environment[$key] = $userdata
}
foreach ($argument in @(
    ...
    "--explorer-output=$relative", '--explorer-seed=2401',
    '--explorer-node=d04_0', '--explorer-mode=play'
)) {
