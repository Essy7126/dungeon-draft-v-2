...
const TARGET := "d04_0"
const TARGET_TITLE := "Le camp des compagnons"
const MANIFEST := "res://data/halts/companions_quarry_v1.json"
...
func _run() -> void:
	...
	if not isolated:
		push_error("Camp probe requires verify.ps1 and isolated user data below artifacts/dev.")
		get_tree().quit(2)
		return
	...
	_check(
		...
		and int(current.get("depth", -1)) == 4 and str(current.get("kind", "")) == "hub",
		current,
	)
	...
	# Damage belongs to the isolated fixture; healing must come from the service.
	var hero: Unit = manager.expedition.character.unit
	hero.current_hp = maxi(1, hero.max_hp.get_int() - 60)
	hero.hp_changed.emit(hero)
	_baseline_hp = hero.current_hp
	_expected_hp = mini(hero.max_hp.get_int(), _baseline_hp + roundi(hero.max_hp.get_int() * 0.30))
	...
	_check("rest_initially_unused", _receipt_count() == 0)
	...
	if landmarks.size() != 2:
		await _finish()
		return
	_check("hearth_is_rest_landmark", str(landmarks[0].get("action", "")) == "rest")
	_check(
		"hearth_has_rest_description",
		not str(landmarks[0].get("description", "")).strip_edges().is_empty(),
	)
	_check("gallery_is_exit_landmark", str(landmarks[1].get("action", "")) == "exit")
	...
	if not _travel(0, "spawn_to_hearth"):
		await _finish()
		return
	_check(
	...
	var rest := _button(hall, "Service_rest")
	_check("rest_available_on_arrival", rest != null and not rest.disabled)
	await _capture("hearth_dialogue_720p")
	await _resize(Vector2i(1920, 1080))
	await _capture("hearth_dialogue_1080p")
	await _press(_button(hall, "Service_rest"), "rest_at_fire")
	_after_secrets = manager.expedition.route.revealed_node_ids.duplicate()
	_check("rest_is_free", int(manager.expedition.gold) == _baseline_gold)
	_check(
		"rest_heals_exact_fraction",
		int(manager.expedition.character.unit.current_hp) == _expected_hp,
	)
	_check("rest_records_one_receipt", _receipt_count() == 1)
	_check_secret_reveal()
	var checkpoint: Dictionary = ExpeditionSaveService.read_snapshot(manager.expedition_save_path)
	_check_checkpoint(checkpoint, "after_rest", "reward")
	await _capture("hearth_rest_receipt")
	await _press(_button(hall, "CloseHaltInteraction"), "close_hearth")
	_check("modal_closed", not hall.interactions.active)
	if not _travel(0, "reopen_hearth"):
		await _finish()
		return
	await _check_used_rest("reopen")
	...
	_check(
		...
		int(manager.expedition.gold) == _baseline_gold and _receipt_count() == 1,
	)
	_check("restored_secret", manager.expedition.route.revealed_node_ids == _after_secrets)
	_check("restored_hp_exact", int(manager.expedition.character.unit.current_hp) == _expected_hp)
	await _capture("production_resumed")
	if not _travel(0, "resumed_spawn_to_hearth"):
		await _finish()
		return
	await _check_used_rest("checkpoint_reload")
	await _press(_button(hall, "CloseHaltInteraction"), "close_before_gallery")
	if not _travel(1, "hearth_to_gallery"):
		await _finish()
		return
	await _capture("gallery_departure_dialogue")
	await _press(_button(hall, "LeavePaintedHalt"), "leave_camp")
	if not await _wait_scene(str(manager.EXPEDITION_SCREEN_PATH)):
		await _finish()
		return
	for frame in 4:
...
func _receipt_count() -> int:
	...
	return receipts.count("rest")


func _check_secret_reveal() -> void:
	_check("rest_does_not_reveal_secrets", _after_secrets == _before_secrets)
...
func _check_checkpoint(checkpoint: Dictionary, label: String, phase: String) -> void:
	...
	_check(label + "_checkpoint_gold", int(saved.get("gold", -1)) == _baseline_gold)
	_check(label + "_checkpoint_receipt_once", used.count("rest") == 1)
	_check(label + "_checkpoint_hp", int(checkpoint.get("current_hp", -1)) == _expected_hp)
	_check(label + "_checkpoint_secret", route.get("revealed_node_ids", []) == _after_secrets)


func _check_used_rest(label: String) -> void:
	var button := _button(hall, "Service_rest")
	_check(label + "_rest_disabled", button != null and button.disabled)
	if button != null:
	...
	var refused: Dictionary = hall.interactions.activate("rest")
	_check(
		label + "_no_repeat_healing",
		int(manager.expedition.character.unit.current_hp) == _expected_hp,
	)
...
func _finish() -> void:
	...
	var report := {
		...
		"limits": [
			...
			"The gallery confirms normal halt departure; no traversal cutscene is added.",
		],
	}
	...
	if file != null:
		...
		passed = false
	print(
		"CAMP_COMPANIONS_VERIFY: %s; %d checks; %d captures"
