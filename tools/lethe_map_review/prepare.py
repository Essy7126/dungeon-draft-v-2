"""Calibrate art against the existing Lethe II geometry, never infer gameplay from pixels."""
import hashlib
import json
...
ROOM = ROOT / 'data/rooms/catabase_routes/route_f51a86b714b9'
OUT = ROOT / 'assets/catabase/combat/lethe_traces_v1'
BASE = ROOT / 'artifacts/dev/lethe-traces/before'
...
def main():
    ...
    reserve=expand(footprint,40)
    shore=expand(footprint,80)
    ...
    for obstacle in geo['obstacles']:
        for cell in obstacle['cells']:
            draw.polygon(polygon(cell),fill='#734854' if obstacle['blocks_line_of_sight'] else '#c99152')
    for cells,color in [(geo['hero_spawns'],'#4297dd'),(geo['enemy_spawns'],'#dd5542')]:
        for cell in cells:
            draw.polygon(polygon(cell),fill=color)
    boat=[150,710,430,845]
