...
func _run() -> void:
	...
	await _settle()
	_check(screen.get_entries().size() == 4, "Three Catabase appearances and the lab trial remain present")
	await _move_pointer(Vector2(2, 2))
	await _capture("selection_1280x720")
	root.size = Vector2i(1920, 1080)
	await _settle()
	await _capture("selection_1920x1080")
	await _click(screen._roster_buttons[1])
	_check(screen.selected_index == 1, "A click through the material selects painted Achille")
	_check(screen.get_selected_entry().get("run").hero_visual_variants == {"achilles": "painted_g"}, "Painted Achille uses Catabase")
	await _capture("painted_1920x1080")
	screen._roster_buttons[3].grab_focus()
	await _settle()
	await _click(screen._roster_buttons[3])
	_check(screen.selected_index == 3, "The scrolling roster reaches the lab adventure")
	_check(screen.get_selected_entry().get("run").resource_path == "res://data/runs/philosopher_trial.tres", "The last card preserves the philosopher adventure")
