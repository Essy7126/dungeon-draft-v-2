...
func _run() -> void:
	...
	await _settle()
	_check(screen.get_entries().size() == 6, "Both Achille appearances and all adventures remain present")
	...
	_check(screen.selected_index == 3, "A click through the material selects Mage")
	...
	screen._roster_buttons[5].grab_focus()
	await _settle()
	await _click(screen._roster_buttons[5])
	_check(screen.selected_index == 5, "The scrolling roster reaches the sixth adventure")
	_check(screen.get_selected_entry().get("run").resource_path == "res://data/runs/philosopher_trial.tres", "The sixth card preserves the philosopher adventure")
