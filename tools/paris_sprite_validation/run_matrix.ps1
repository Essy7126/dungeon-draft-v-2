param(
    ...
    [string[]]$Scenarios = @('spectral', 'ice', 'fire', 'vortex', 'teleport', 'approach', 'transform', 'defeat'),
    [string]$Batch = 'matrix',
    [switch]$Capture
)
...
foreach ($scenario in $Scenarios) {
    if ($scenario -notin @('spectral', 'ice', 'fire', 'vortex', 'teleport', 'approach', 'transform', 'defeat')) {
        throw "Unknown scenario $scenario"
    }
}
$results = @()
$relativeRoot = "artifacts/paris_sprite_validation_v1/$Batch"
...
foreach ($direction in $Directions) {
    foreach ($scenario in $Scenarios) {
        ...
        $arguments = @('--path', $repo, '--resolution', '1200x800',
            'res://tools/paris_sprite_validation/ParisCombatValidation.tscn', '--',
