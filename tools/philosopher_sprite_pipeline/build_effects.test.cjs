...
async function fixture(t, alteration = '') {
  ...
  const directory = fs.mkdtempSync(path.join(base, 'philosopher-effects-pipeline-'));
  t.after(() => {
    const resolved = path.resolve(directory);
    assert.equal(path.dirname(resolved), base);
    assert.match(path.basename(resolved), /^philosopher-effects-pipeline-/);
...
test('grid extraction keeps detached alpha and one stable transform per row', async t => {
  ...
  assert.deepEqual(result.manifest.animation_order, ['bolt', 'impact', 'heal', 'control', 'shield', 'repel']);
...
test('resource references exactly 24 frames and six non-looping authored animations', () => {
  const resource = pipeline.resourceText('res://assets/vfx/philosopher_mage/sprites_v1/effects.png');
  assert.equal((resource.match(/\[sub_resource type="AtlasTexture"/g) || []).length, 24);
  assert.equal((resource.match(/"loop": false/g) || []).length, 6);
  for (const name of ['bolt', 'impact', 'heal', 'control', 'shield', 'repel']) {
...
test('inspect writes nothing and invalid builds do not replace an existing atlas', async t => {
  const input = await fixture(t);
  const relative = `artifacts/philosopher_effects_pipeline_test_${process.pid}`;
  ...
  t.after(() => {
    assert.equal(path.dirname(output), path.join(ROOT, 'artifacts'));
    assert.match(path.basename(output), /^philosopher_effects_pipeline_test_\d+$/);
