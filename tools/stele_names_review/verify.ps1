...
$root=Get-DevRoot
$runRoot=New-DevRun 'stele-names-production'
$engineLock=$null
try {
    ...
    $arguments=@('--verbose','--single-window','--path',$root,'--rendering-method','gl_compatibility','--audio-driver','Dummy','--position','-3000,-3000','--resolution','1920x1080','--log-file',(Join-Path $runRoot 'production.engine.log'),'res://tools/stele_names_review/VerifySteleNames.tscn','--',"--production-output=$runRoot")
