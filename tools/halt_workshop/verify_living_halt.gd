...
const BackdropService := preload("res://addons/dungeon_draft_arena_studio/services/arena_backdrop_transaction_service.gd")
const BackdropSource := preload("res://addons/dungeon_draft_arena_studio/domain/arena_backdrop_source_definition.gd")
...
func _run() -> void:
	...
	for size: Vector2i in [Vector2i(1280, 720), Vector2i(1920, 1080)]:
		...
		add_child(hall)
		for attempt in 180:
			if hall.is_ready_for_play(): break
		...
		_source_evidence = {"manifest": hall.manifest_path,
			...
			"build_sha256": FileAccess.get_sha256(str(hall.definition.build_dir).path_join("build.json"))}
		var source := BackdropSource.new()
		source.background_path = str(hall.definition.source.image)
		source.source_image_size = Vector2i(hall.definition.source.size[0], hall.definition.source.size[1])
		var arena := Arena.new()
		arena.source_image_size = source.source_image_size
		var inspection := BackdropService.new().inspect(arena, source, BackdropService.CopyMode.BACKGROUND_ONLY)
		_check("studio_backdrop_loadable", bool(inspection.get("ok", false)))
		_check("studio_source_dimensions", inspection.get("actual_image_size", Vector2i.ZERO) == source.source_image_size)
		await _capture("arrival")
		var frame: Dictionary = hall.player.get_visual_state()
		_check("achilles_visual_ready", bool(frame.visible) if frame.has("visible") else hall.player.is_visual_ready())
		...
		for step in 20: hall.advance_world(1.0 / 60.0)
		...
		hall.stop_movement()
		_check("stop_idle", not hall.is_player_moving() and str(hall.player.get_visual_state().stem) == "idle")
		hall.set_zoom(1.5)
		var screen: Vector2 = hall.world.to_global(hall.player.position)
		_check("zoom_coordinate_roundtrip", hall.world.to_local(screen).distance_to(hall.player.position) < 0.01)
		await _capture("zoom")
		hall.set_zoom(1.0)
		if _rendered: await _test_rendering()
		hall.queue_free()
		await get_tree().process_frame
	_check("all_movement_samples_safe", samples > 0 and unsafe == 0)
	if "--record" in OS.get_cmdline_user_args() and _rendered:
		await _record()
	var failures: Array = checks.filter(func(c: Dictionary): return not c.passed)
	var report := {"passed": failures.is_empty(), "check_count": checks.size(), "checks": checks,
		"failures": failures, "captures": captures, "movement_samples": samples, "unsafe_samples": unsafe,
		"rendered": _rendered, "scene": "res://hub/painted_halt/LivingHalt.tscn",
		"date": Time.get_datetime_string_from_system(true), "source_evidence": _source_evidence}
	var file := FileAccess.open(output.path_join("verification.json"), FileAccess.WRITE)
	file.store_string(JSON.stringify(report, "\t"))
	print("LIVING_HALT_VERIFY: %s; %d checks, %d captures, %d movement samples, %d unsafe" % [failures.is_empty(), checks.size(), captures.size(), samples, unsafe])
...
func _walk(target: Vector2, label: String) -> void:
	...
	for step in 2400:
		if not hall.is_player_moving(): break
		var before: Vector2 = hall.player.position
		hall.advance_world(1.0 / 60.0)
		samples += 1
		if not hall.nav.is_walkable(hall.player.position) or before.distance_to(hall.player.position) > 5.0:
			unsafe += 1
...
func _check(label: String, passed: bool, details: Variant = null) -> void:
	...
	if not passed: push_error("HALT_CHECK: " + label + " " + str(details))


func _capture(label: String) -> void:
	if not _rendered: return
	await get_tree().process_frame
...
func _test_rendering() -> void:
	...
	hall.player.hide()
	for id: String in hall.layers: hall.set_layer(id, false)
	...
	_check("water_click_ripple", hall.trigger_ripple(hall.point(hall.definition.review.water_pixel)))
	_check("stone_rejects_ripple", not hall.trigger_ripple(hall.point(hall.definition.review.stable_pixel)))
	...
	change = _difference(first, second, hall.definition.cascades[0].splash, 35) if not hall.definition.cascades.is_empty() else 0.0
	_check("atmosphere_spray_moves", change > 0.0001, change)
	for id: String in hall.layers: hall.set_layer(id, true)
	hall.set_paused(true)
	...
	hall.reduced = true
	hall.advance_world(0.0)
	_check("reduced_effect_strength", is_equal_approx(float(hall.effect_material.get_shader_parameter("strength")), 0.28))
	hall.reduced = false
	hall.set_original(true)
	first = await _image_at(4.0)
	second = await _image_at(6.0)
	_check("original_water_stable", _difference(first, second, hall.definition.review.water_pixel) == 0.0)
	_check("original_fire_stable", _difference(first, second, hall.definition.review.fire_pixel) == 0.0)
...
func _record() -> void:
	...
	for attempt in 180:
		if hall.is_ready_for_play(): break
		await get_tree().process_frame
	if not hall.is_ready_for_play(): return
	hall.set_process(false)
	hall.set_chrome_visible(false)
	...
	for frame in 336:
		...
		await RenderingServer.frame_post_draw
		var image := get_viewport().get_texture().get_image()
		if image.save_jpg(ProjectSettings.globalize_path(frames.path_join("frame_%04d.jpg" % frame)), 0.95) != OK:
