...
const root=process.cwd(),out=path.join(root,'artifacts/spine_trial/veilleur_walk_E_v2');
...
const report={date:new Date().toISOString(),url:'http://127.0.0.1:8734/files/veilleur_walk_E_v2/review.html',checks:[],errors:[]};
...
try{
 ...
 await page.waitForFunction(()=>window.reviewReady===true);await page.evaluate(()=>window.seekWalk(0));
 const poses=[];const pictures=[];
 for(let i=0;i<48;i++){
  await page.evaluate(i=>window.seekWalk(i),i);
  poses.push(await page.evaluate(()=>({before:Number(document.getElementById('before').dataset.frame),after:Number(document.getElementById('after').dataset.frame)})));
  pictures.push(await page.$eval('#after',c=>c.toDataURL()));
 }
 check('48 synchronized comparison states',poses.every((p,i)=>p.before===Math.floor(i/2)&&p.after===i));
 ...
 await page.select('#speed','1');await page.click('#restart');await page.click('#play');await page.waitForFunction(()=>window.walkState.elapsed>.2);await page.click('#play');
 ...
 await page.select('#background','sources');await page.evaluate(()=>window.seekWalk(24));
