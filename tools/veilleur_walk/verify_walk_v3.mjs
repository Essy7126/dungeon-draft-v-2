...
const root=process.cwd(),out=path.join(root,'artifacts/spine_trial/veilleur_walk_E_v3');
...
const report={date:new Date().toISOString(),url:'http://127.0.0.1:8734/files/veilleur_walk_E_v3/review.html',checks:[],errors:[]};
...
try{
 ...
 check('Initial autoplay',await page.evaluate(()=>window.walkState.playing&&window.walkState.frame>=0&&window.walkState.frame<32));
 await page.evaluate(()=>window.seekWalk(0));
 const poses=[],pictures=[];
 for(let i=0;i<32;i++){
  await page.evaluate(i=>window.seekWalk(i),i);
  poses.push(await page.evaluate(()=>({reference:Number(document.getElementById('referenceCycle').dataset.frame),hero:Number(document.getElementById('hero').dataset.frame)})));
  pictures.push(await page.$eval('#hero',c=>c.toDataURL()));
 }
 check('32 synchronized comparison states',poses.every((p,i)=>p.reference===Math.floor(i/2)&&p.hero===i));
 check('32 distinct rendered frames',new Set(pictures).size===32);
 await page.click('#next');check('Forward wrap',await page.evaluate(()=>window.walkState.frame===0));
 await page.click('#prev');check('Reverse wrap',await page.evaluate(()=>window.walkState.frame===31));
 ...
 for(const background of ['sources','cendres','sanctuaire']){
  ...
  check(`Map ${background}`,await page.$eval('#floor',(c,w)=>c.dataset.background===w,background));
