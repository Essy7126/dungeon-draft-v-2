...
const root=process.cwd(),out=path.join(root,'artifacts/spine_trial/veilleur_walk_E_v4');
...
const report={date:new Date().toISOString(),url:'http://127.0.0.1:8734/files/veilleur_walk_E_v4/review.html',checks:[],errors:[]};
...
try{
 ...
 check('48 synchronized comparison states',poses.every((p,i)=>p.reference===Math.floor(((i/48+.5)%1)*32+1e-7)&&p.hero===i));
 check('48 distinct rendered frames',new Set(pictures).size===48);
 await page.evaluate(()=>window.seekWalk(12));
 await page.select('#timing','native');
 check('Native cadences',await page.evaluate(()=>Number(document.getElementById('before').dataset.frame)===29&&window.walkState.timing==='native'));
 await page.select('#timing','phase');
 check('Aligned contact phases',await page.evaluate(()=>Number(document.getElementById('before').dataset.frame)===24));
