...
func run() -> void:
	var scene: PackedScene = load("res://tools/passe_rive_unarmed/WalkReview.tscn")
	var lab = scene.instantiate()
	root.add_child(lab)
	await process_frame
	lab.set_process(false)
	for d: String in ["E", "S", "N", "W"]:
		check(lab.textures[d].size() == 60, "60 loaded frames: " + d)
		lab.direction = d
		for i in range(60):
			lab.phase = float(i) / 60.0 + 0.000001
			lab.refresh()
	...
	var report := {
		...
		"calibration": "projected heel/toe contact anchors",
	}
