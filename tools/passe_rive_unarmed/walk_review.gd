...
const ROOT := "res://artifacts/spine_trial/passe_rive_walk_unarmed_v2/"
...
func _build_controls() -> void:
	...
	help.text = "Cliquer une case pour marcher · Flèches : un pas · Espace : pause · Marche sans armes · nouvelles pièces 2D."
	add_child(help)
...
func advance(delta: float) -> void:
	...
	while remaining > 0.000001 and not route.is_empty():
		...
		var speed := cycle_distance / float(data.duration)
		...
		phase = fposmod(phase + used / float(data.duration), 1.0)
		remaining -= used
...
func refresh() -> void:
	sprite.texture = textures[direction][
		mini(int(data.frame_count) - 1, floori(phase * data.frame_count))
	]
	sprite.position = actor_position
	label.text = "PASSE-RIVE — MARCHE SANS ARMES · RÉVISION 2    %s · image %02d/%d" % [
		direction,
		floori(phase * data.frame_count) + 1,
