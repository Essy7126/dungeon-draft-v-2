...
const CASES := preload("res://tools/achilles_animation_polish_v3_validation/cases.gd")
const PROBE := preload("res://tools/achilles_animation_polish_v3_validation/polish_probe.gd")
...
func _launch() -> void:
	var configuration := {"kit": "base", "scenario": "", "direction": "E",
		"room_path": "res://data/arenas/greek_drawn_courtyard_v1/arena.tres", "walk_cells": 3, "appearance": "classic"}
	for argument: String in OS.get_cmdline_user_args():
		for key: String in ["kit", "scenario", "direction", "room_path", "walk_cells", "appearance"]:
	...
	var placement := CASES.configure_placement_fixture(arena, configuration.scenario,
		configuration.kit, configuration.direction, int(configuration.walk_cells))
	probe.placement = placement
	if not bool(placement.get("ok", false)):
		_fail(probe, str(placement.get("error", "placement_fixture_failed")))
		return
	var run := RunData.new()
	run.run_name = "Achilles animation polish v3 validation"
	...
	if CASES.KITS[configuration.kit].has("exp_spell"):
		var classifications := CombatActionClassificationCatalogData.new()
		classifications.catalog_id = &"validation_actual_catabase_actions"
		classifications.entries.assign(ExpeditionBuildCatalog.new().get_action_classifications())
		run.action_classification_catalog = classifications
	if configuration.appearance == "painted_g":
		run.hero_visual_variants = {"achilles": "painted_g"}
	elif configuration.appearance != "classic":
		_fail(probe, "unknown_appearance")
		return
