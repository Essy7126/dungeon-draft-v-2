...
$root = Get-DevRoot
$runRoot = New-DevRun 'gue-serments-review'
$room = 'res://data/rooms/catabase_routes/route_1c7d4c943b3a/room.tres'
$results = @()
$lock = $null
try {
    ...
    foreach ($size in @('1920x1080', '1200x896')) {
        ...
        $arguments = @(
            ...
            "--capture=$relative/$label.png", "--resolution=$size", '--production-qa', '--gue-serments-visual-review'
        )
        if ($size -eq '1200x896') {
        ...
        if (-not $passed) { throw "Ford review failed: $reportPath" }
    }
