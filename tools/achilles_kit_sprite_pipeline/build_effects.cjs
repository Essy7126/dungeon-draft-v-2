...
const SOURCE = 'art/source/vfx/achilles_kit_v2/source_effects.png';
const OUTPUT = 'assets/vfx/achilles_kit_v2';
const layoutTools = require('./effects_source_layout.cjs');
const ANIMATIONS = ['arrow', 'impact', 'sweep', 'guard', 'dust', 'barrier'];
...
"texture": SubResource("Frame_${name}_${index}")
    ...
    const speed = name === 'arrow' ? 20.0 : name === 'impact' ? 4 / 0.22 : 4 / 0.24;
    return `{
"frames": [${frames}],
"loop": false,
"name": &"${name}",
...
async function prepare(sourcePath, authoredLayout = null) {
  ...
  const manifest = {
    ...
    processing: {
      ...
      arrow_orientation: 'Authored horizontal, pointing right; never flipped or rotated by this pipeline.',
    },
...
async function build(settings) {
  ...
  const defaultLayout = path.join(__dirname, 'source_layout.json');
...
if (require.main === module) {
  (async () => {
    ...
    if (settings.help) {
      console.log('node tools/achilles_kit_sprite_pipeline/build_effects.cjs [--inspect] [--source path.png] [--output project/directory] [--layout path.json]');
      return;
    }
...
module.exports = { options, analyze, extractRaw, verifyCell, rowCrop, packRaw, resourceText, outputLocation, prepare, build };
