...
$root = Get-DevRoot
$runRoot = New-DevRun 'puits-sources-review'
$room = 'res://data/rooms/catabase_routes/route_9c104c2158d3/room.tres'
$results = @()
$lock = $null
try {
    ...
    foreach ($size in @('1920x1080', '1200x896')) {
        ...
        $arguments = @(
            ...
            "--capture=$relative/$label.png", "--resolution=$size", '--production-qa', '--puits-sources-visual-review'
        )
        if ($size -eq '1200x896') {
        ...
        if (-not $passed) { throw "Sources review failed: $reportPath" }
    }
