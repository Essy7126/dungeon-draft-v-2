...
func _run() -> void:
	...
	for viewport_size in SIZES:
		...
		await _settle()
		_check(session.cards.hand.size() == 5, "five-card capacity fixture", viewport_size)
		await _capture("cards_combat_five", viewport_size, [hand])
		session.character.loadout.resize_slots(6)
		session.cards.start_turn()
		await _settle()
		_check(session.cards.hand.size() == 6, "six-card capacity fixture", viewport_size)
		...
		await _settle()
		_check(hud_band.visible and hand.visible and session.cards.hand.size() == 6, "returning from objects preserves the hand and common HUD", viewport_size)
		...
		await _settle()
		_check(actor.current_ap == original_ap - 1 and session.cards.hand != previous_hand and session.cards.hand.size() == 6, "real recompose button exchanges one card for exactly one AP", viewport_size)
		...
		_check(_resolve_advancement(session), "resolve level before loot", viewport_size)
		...
		_check(collection.find_children("CardFamily_weapon_gesture", "PanelContainer", true, false).size() == 1, "six Gestures are grouped into a single family panel", viewport_size)
		var local_picker := collection.find_child("CardReplacementTarget", true, false) as OptionButton
		_check(local_picker != null and local_picker.item_count <= 4, "replacement groups outgoing copies by family", viewport_size)
