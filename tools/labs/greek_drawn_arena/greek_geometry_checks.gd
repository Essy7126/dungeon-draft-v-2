...
static func _semantic_pit_annotation(arena: ArenaDefinition, grid: GridData, floor: Dictionary, renderer: ArenaTerrainVisualRenderer) -> Dictionary:
	...
	var cells := {}
	var groups := 0
	var path := arena.resource_path.get_base_dir().path_join("geometry_manifest.json") \
		if not arena.resource_path.is_empty() else "res://data/arenas/greek_drawn_courtyard_v1/geometry_manifest.json"
