...
func run(battle: Node, hero: Unit, grid: GridData, pathfinder: Pathfinder, view: Node2D, renderer: ArenaTerrainVisualRenderer) -> Dictionary:
	...
	var spell_result := {}
	if guard == null:
		errors.append("canonical_guard_spell_missing")
		...
		var expected_ap_cost := hero.get_spell_ap_cost(guard)
		...
		if hero.current_ap != before_ap - expected_ap_cost or hero.current_shield != maxi(before_shield, guard.shield_grant) \
				or hero.get_spell_availability_reason(guard) != &"once_per_activation" or hero.get_spell_uses(guard) != before_uses + 1 or hero.get_spell_cooldown_remaining(guard) != 0 or state.current != TurnState.State.IDLE:
