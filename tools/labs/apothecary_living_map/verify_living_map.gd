...
const SCENE_PATH := "res://tools/labs/apothecary_living_map/LivingApothecary.tscn"
const OUTPUT_DIR := "res://artifacts/apothecary_living_map"
const NATIVE_SIZE := Vector2(1376, 768)
const WATER_ROI := Rect2(750, 268, 65, 17)
const STONE_ROI := Rect2(530, 480, 60, 45)
const FIRE_ROI := Rect2(253, 331, 29, 37)
const RIPPLE_ROI := Rect2(716, 253, 147, 67)
const WATER_POINT := Vector2(788, 274)
const STONE_POINT := Vector2(600, 500)
...
func _run() -> void:
	...
	for failure: String in _failures:
		push_error("APOTHECARY_LIVING_MAP: " + failure)
	var result := "FAIL" if not _failures.is_empty() else ("PASS_RENDERED" if _capture_enabled else "PASS_API_ONLY")
	print("APOTHECARY_LIVING_MAP: %s (%d checks; %d captures). Report: %s/verification.json" % [result, _checks.size(), _captures.size(), OUTPUT_DIR])
...
func _exercise_rendered_effects() -> void:
	...
	_assert_motion("water_animates_in_basin", water_a, water_b, WATER_ROI)
	...
	_assert_motion("candle_flame_animates", fire_a, fire_b, FIRE_ROI)
	...
	# Local candle light may illuminate stone. The water-only assertion above
...
func _exercise_ripple_input() -> void:
	...
	_check("ripple_accepts_water", bool(_preview.call("trigger_ripple", WATER_POINT)))
	_preview.call("set_effect_time", 2.35)
	if _capture_enabled:
...
func _write_report() -> void:
	var report := {
		...
		"captures": _captures,
		"method": "Instantiates the real preview scene after autoloads. Public API and mouse-event smoke tests run at 1280x720 and 1920x1080. With --capture and an active renderer, fixed effect times 0.25/1.55 produce framebuffer comparisons in independent native image regions: basin, candle and stone. Ripple comparison holds shader time equal. Shader validation is never claimed for headless runs.",
