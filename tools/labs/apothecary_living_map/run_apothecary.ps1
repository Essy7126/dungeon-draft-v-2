...
$apothecaryProjectRoot = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '../../..')).Path
if (-not $GodotPath) {
    $apothecaryCandidates = @(
    ...
    $GodotPath = $apothecaryCandidates | Where-Object { Test-Path -LiteralPath $_ -PathType Leaf } | Select-Object -First 1
}
...
& $GodotPath --path $apothecaryProjectRoot --rendering-method gl_compatibility --resolution $Resolution --scene res://tools/labs/apothecary_living_map/LivingApothecary.tscn
exit $LASTEXITCODE
