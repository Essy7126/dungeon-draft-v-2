...
$root = Get-DevRoot
$runRoot = New-DevRun 'puits-descent-review'
$room = 'res://data/rooms/catabase_routes/route_76ac69bb7c8d/room.tres'
$results = @()
$lock = $null
try {
    ...
    foreach ($size in @('1920x1080', '1200x896')) {
        ...
        $arguments = @(
            ...
            "--capture=$relative/$label.png", "--resolution=$size", '--production-qa', '--puits-visual-review'
        )
        if ($size -eq '1200x896') {
