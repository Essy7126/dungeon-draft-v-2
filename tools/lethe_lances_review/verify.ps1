...
$root = Get-DevRoot
$runRoot = New-DevRun 'lethe-lances-review'
$room = 'res://data/rooms/catabase_routes/route_10ce1ff88330/room.tres'
$results = @()
$lock = $null
try {
    ...
    foreach ($size in @('1920x1080', '1200x896')) {
        ...
        $arguments = @(
            ...
            "--capture=$relative/$label.png", "--resolution=$size", '--production-qa', '--lances-visual-review'
        )
        if ($size -eq '1200x896') {
        ...
        if (-not $passed) { throw "Lances review failed: $reportPath" }
    }
