...
$root = Get-DevRoot
$run = New-DevRun 'braises-cloitre-prepare'
$lock = $null
try {
    ...
    $prepare = Invoke-DevProcess $godot @('--headless','--path',$root,'--log-file',(Join-Path $run 'prepare.engine.log'),'res://tools/braises_cloitre_review/PrepareRoom.tscn') $root $run 'prepare' 90 $environment
    $errors = @(Get-DevEngineErrors $run 'prepare' | Sort-Object -Unique)
    $text = [IO.File]::ReadAllText((Join-Path $run 'prepare.stdout.log'))
    if (-not (Test-DevProcessSuccess $prepare) -or $errors.Count -gt 0 -or $text -notmatch 'BRAISES_CLOITRE_PREPARED:.*"ok":true') { throw "Preparation failed: $($errors -join ' | ')" }
