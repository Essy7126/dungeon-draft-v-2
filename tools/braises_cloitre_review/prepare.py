"""Calibrate art against Les braises du cloître geometry, never infer gameplay from pixels."""
import hashlib
import json
...
ROOM = ROOT / 'data/rooms/catabase_routes/route_edce0087c741'
OUT = ROOT / 'assets/catabase/combat/braises_cloitre_v1'
BASE = ROOT / 'artifacts/dev/braises-cloitre/before'
...
def main():
    ...
    # Arrival arch beyond the tactical floor.
    ...
    data={'canvas_size':[1920,1200],'allowed_floor_polygon':reserve,
          'shore_polygon':shore,'arrival_region':[120,170,350,405],'floor_count':len(floor),
          ...
          'legend':'Green cells: engine floor. Red/purple/orange/blue: engine-owned spawns/props. Paint ALL reserve flat, without tiles or props. Upper left rectangle: arrival arch from previous room, outside combat. Lava only outside the gray bank.'}
