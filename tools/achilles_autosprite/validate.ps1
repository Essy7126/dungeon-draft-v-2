...
$runRoot = New-DevRun 'achilles-autosprite'
...
$scripts = @('res://test/unit/test_achilles_autosprite.gd')
if ($Integration) {
    $names = @('test_achilles_sprite_*.gd','test_achilles_kit_sprite_*.gd','test_achilles_kit_movement_v2.gd','test_achilles_kit_reaction_arrival.gd','test_achilles_polish_v3_assets.gd','test_achilles_painted_g_runtime.gd','test_achilles_guard_sprite_vfx.gd','test_achilles_odyssey_production_contract.gd','test_painted_halt_runtime.gd','test_unit_movement_presentation.gd')
...
if ($Full) {
    $scripts = @(Get-ChildItem (Join-Path $taskRepo 'test/unit') -Filter 'test_achilles_*.gd' | ForEach-Object { 'res://test/unit/' + $_.Name })
...
Write-DevJson $inputPath @{schema_version=1;suite_id='achilles-autosprite';godot=@{path=$godot;version=$version};gut_version=$toolchain.gut_version;selection=@{directory='res://test/unit';prefix='test_';suffix='.gd';include_subdirectories=$false;scripts=$scripts};expected=@{minimum_tests=7;failures=@()};process=@{import=$import;gut=$gut};files=@{import_stdout='import.stdout.log';import_stderr='import.stderr.log';gut_stdout='gut.stdout.log';gut_stderr='gut.stderr.log';junit='gut.junit.xml'}}
