...
$runRoot=New-DevRun 'spine-godot-preview'
...
try {
    ...
    $arguments=@('--path',$projectRoot,'--rendering-method','gl_compatibility','--audio-driver','Dummy','--position','-3000,-3000','--resolution','1200x896','res://tools/spine_trial/SpineTrial.tscn','--',('--report='+$reportPath))
    $process=Invoke-DevProcess (Resolve-DevGodot) $arguments $projectRoot $runRoot 'preview' 45
    ...
    $passed=(Test-DevProcessSuccess $process) -and $errors.Count -eq 0 -and $report.passed -and $report.checks.Count -eq 3
