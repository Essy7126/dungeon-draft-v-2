...
## A support turn is planned without mutating HP, AP, statuses or occupancy.
...
static func _score(ai, enemy: Unit, spell: Spell, target: Unit, units: Array, origin: Vector2i, state: Dictionary) -> float:
	...
	var cell := origin if target == enemy else target.grid_pos
	if spell.is_healing():
		if state.healed.has(key) or target.get_hp_ratio() >= enemy.ai_profile.support_heal_threshold:
			return 0.0
		var missing := target.max_hp.get_int() - target.current_hp
		return 200.0 + mini(missing, spell.heal) if missing >= 8 else 0.0
	if spell.shield_grant > 0:
		if state.shielded.has(key) or target.current_shield >= spell.shield_grant / 2:
			return 0.0
		var threat_distance := _nearest_opponent_distance(grid, target.team, cell, units)
		if threat_distance > enemy.ai_profile.support_protection_distance:
			return 0.0
		return 140.0 + (1.0 - target.get_hp_ratio()) * 25.0 - threat_distance
	if spell.push_distance > 0:
		return 180.0 + Terrain.push_bonus(ai, target, origin, spell.push_distance) \
			if grid.manhattan(origin, cell) == 1 else 0.0
	if spell.applied_status != null and spell.applied_status.mp_reduction > 0:
		if state.controlled.has(key) or target.has_status(spell.applied_status.get_effective_status_id()) \
				or target.max_mp.get_int() <= 0:
			return 0.0
		return 100.0 + maxi(0, 5 - grid.manhattan(origin, cell))
	if spell.deals_damage():
		var expected: int = ai._expected_spell_damage(enemy, spell, target)
		return 70.0 + (30.0 if expected >= target.current_hp + target.current_shield else 0.0) \
			+ (1.0 - target.get_hp_ratio()) * 10.0
	return 0.0
