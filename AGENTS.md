...
- État du produit : `README.md`, puis seulement la référence utile dans `docs/current/` (produit, architecture, validation, contenu).
...
- `./dev.ps1 context catabase|cards|combat|studio|terrain|personnages` : points d’entrée ; `-Kind code|tests|docs|data|art`, `-Page 2`, `-IncludeArchive` pour affiner.
- `./dev.ps1 test smoke|cards|catabase|monsters|terrain|studio|all` ou un chemin exact ; `smoke` ne couvre que les codex UI.
...
- Les notes datées et anciens manifestes sont historiques ; le statut des personnages/maps est dans `docs/current/content.md`. Ne pas assimiler « hors menu public » à « inutilisé ».
